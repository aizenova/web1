...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'upload_avatar') {
    ...
    // filename: user_{id}_{timestamp}.{ext} or user_guest_{timestamp}
    $uid = $_SESSION['user_id'] ?? 'guest';
    $ext = $allowed[$type];
    $filename = 'user_' . preg_replace('/[^a-z0-9_-]/i', '', $uid) . '_' . time() . '.' . $ext;
    ...
    flash('success', 'Foto profil berhasil diunggah.');
    redirect('account.php');
}
